...
gitGraph
    ...
    commit id: "tag: llvmorg-23.1.0"
    checkout main
    branch rust-lang/llvm-project
    commit id: "branch: rustc/23.1-2026-07-22"
    checkout llvm/llvm-project
    merge rust-lang/llvm-project id: "diff: llvmorg-23.1.0 ... rustc/23.1-2026-07-22 (branch up to date)"
    checkout llvm/llvm-project
    checkout main
    branch KomiMoe/Arkari
    commit id: "branch: llvm-23.x"
    checkout llvm/llvm-project
    merge KomiMoe/Arkari id: "diff: llvmorg-23.1.0 ... llvm-23.x"
    checkout main
    commit id: "base: llvmorg-23.1.0"
...
- On Linux (with ninja)
```bash
cmake -S llvm -B build -GNinja -DCMAKE_INSTALL_PREFIX="./Release" -DLLVM_ENABLE_PROJECTS="" -DLLVM_TARGETS_TO_BUILD="X86" -DLLVM_INSTALL_UTILS=ON -DLLVM_INCLUDE_TESTS=OFF -DLLVM_BUILD_TESTS=OFF -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_BUILD_BENCHMARKS=OFF -DLLVM_INCLUDE_EXAMPLES=OFF -DLLVM_ENABLE_BACKTRACES=OFF -DLLVM_BUILD_DOCS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release
```

- On Windows (with MSBuild)
```bash
cmake -S llvm -B build -DCMAKE_INSTALL_PREFIX="./Release" -DLLVM_ENABLE_PROJECTS="" -DLLVM_TARGETS_TO_BUILD="X86" -DLLVM_INSTALL_UTILS=ON -DLLVM_INCLUDE_TESTS=OFF -DLLVM_BUILD_TESTS=OFF -DLLVM_INCLUDE_BENCHMARKS=OFF -DLLVM_BUILD_BENCHMARKS=OFF -DLLVM_INCLUDE_EXAMPLES=OFF -DLLVM_ENABLE_BACKTRACES=OFF -DLLVM_BUILD_DOCS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded
```

## Used Version
- [repo `rust-lang/llvm-project` branch `rustc/23.1-2026-07-22`](https://github.com/rust-lang/llvm-project/commit/ea7d852a70e8bdfaf601d6626a760f9771b2c4b4)
- [repo `komimoe/Arkari` branch `llvm-23.x`](https://github.com/komimoe/Arkari/commit/5200fe2df127f52b6a9d34b9dde0c8dfe01c1b3b)
